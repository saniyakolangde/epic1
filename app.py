...
geo_data['Council Region'] = geo_data['name'].str.replace('City of ',
                                                          '').str.strip()

# Convert the CSV data to the appropriate format
csv_data = csv_data.melt(id_vars='Council Region', var_name='Country',
                         value_name='Count')
...
merged_geo_data['Count'] = pd.to_numeric(merged_geo_data['Count'],
                                         errors='coerce')
...
merged_geo_data = merged_geo_data.rename(
    columns={'name': 'City', 'Count': 'People'})
...
map_fig = px.choropleth_mapbox(
    ...
    # This key matches the name property in the GeoJSON
    color='People', hover_name='Council Region',
    color_continuous_scale="Viridis",
    title='Regions with People from Selected Countries',
    mapbox_style="carto-positron", center={"lat": -37.8136, "lon": 144.9631},
    zoom=8, opacity=0.8
)
    ...
    html.Div([
        dcc.Dropdown(
            ...
            options=[{'label': country, 'value': country} for country in
                     ['', 'Afghanistan', 'Iran', 'Myanmar', 'Iraq', 'Pakistan',
                      'Thailand',
                      'Sri Lanka', 'Malaysia', 'India', 'Lebanon', 'Turkey',
                      'Cambodia',
                      'Egypt', 'Papua New Guinea', 'Indonesia']],
            value='All'
        )], style={'width': '40%', 'marginRight': '20px'}),
    # Dropdown width and margin to separate from the graph
...
def update_map(value):
    if value:
        ...
        filtered_geo_data = merged_geo_data[
            merged_geo_data['Country'].str.title() == selected_country]
        map_fig = px.choropleth_mapbox(
            filtered_geo_data, geojson=geojson_data,
            locations=filtered_geo_data['City'],
            featureidkey="properties.name",
            color='People', hover_name='Council Region',
            color_continuous_scale="Viridis",
            title=f'Regions with People from {selected_country}',
            mapbox_style="carto-positron",
            center={"lat": -37.8136, "lon": 144.9631}, zoom=8, opacity=0.8
        )

        if filtered_geo_data.empty:
            # Handle the case where there is no data to display
            return px.choropleth_mapbox(
                merged_geo_data, geojson=geojson_data,
                locations=merged_geo_data['City'],
                featureidkey="properties.name",
                color='People', hover_name='Council Region',
...
def update_pie_chart(clickData, selected_country):
    if clickData:
        ...
        top_languages = get_top_five_language(selected_region,
                                              selected_country)
        country_language = get_country_language(selected_country,
                                                country_languages)
        print(top_languages)

        filtered_data = csv_data[(csv_data['Country'] == selected_country) & (
                    csv_data['Council Region'] == selected_region)]

        csv_data['Count'] = pd.to_numeric(csv_data['Count'], errors='coerce')

        total_count = csv_data[csv_data['Country'] == selected_country][
            'Count'].sum()
        ...
        for lang in top_languages:
            ...
            if percentage < 1:
                tip_text.append(
                    f"There are {lang['count']} {lang['language']} speaking people in {selected_region}")
        ...
        pulls = [0.2 if label == country_language else 0 for label in labels]
...
def get_top_five_language(region_name, country_name):
    ...
    try:
        country_language = get_country_language(country_name,
                                                country_languages)
        # Read the CSV file
        ...
        region_row = df[df[
                            'Region'].str.strip().str.lower() == region_name.strip().lower()]
        ...
        return [{"language": language, "count": count} for language, count in
                top_five]
